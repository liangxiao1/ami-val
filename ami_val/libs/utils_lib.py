...
<html lang="en">
<head>
  <title>AMI-VAL Test Report</title>
  <meta charset="utf-8">
...
<div class="container-fluid">
  <h2>AMI-VAL Test Report</h2>
  <p>This report is generated by <a href='https://pypi.org/project/ami-val/'>ami-val</a> for displaying RHEL AMIs validation result.</p>   
  <hr class="solid">       
    '''
    table_header = '''
...
$(document).ready(function() {
  $('#result').DataTable();
    ...
    '''
    with open(logdir + '/sum.html', 'a+') as fh:
        ...
        for ami in sums:
            ...
            for r in ami['result']:
                ...
                if 'PASS' in result:
                    ...
                    summary['fail'] += 1
                summary['total'] += 1
    pass_rate = (summary['pass'])/(summary['total'] - summary['skip'])*100
    summary = """
        <!-- Table -->
        <table class="table table-bordered">
            <tr>
            <td>Summary</td><td>Total:<b>{}</b>, PASS:<b class="text-success">{}</b>, FAIL:<b class="text-danger">{}</b>, SKIP:<b class="text-warning">{}</b>, ERROR:<b class="text-danger">{}</b> - success rate of <b>{}</b>% (excluding SKIP and CANCEL)</tr>
            <tr>
            <td>Regions Uploaded</td><td>{}</td></tr>
            <tr>
            <td>Regions Missed</td><td>{}</td></tr>
        </table>
    """.format(summary['total'],summary['pass'],summary['fail'],summary['skip'],summary['error'],format(pass_rate,'0.2f'),region_uploaded, region_missed)
    with open(logdir + '/sum.html', 'a+') as fh:
        fh.write(summary)
    with open(logdir + '/sum.html', 'a+') as fh:
    ...
    with open(logfile, 'r') as fh:
        ...
        for ami in sums:
            ...
            for r in ami['result']:
                ...
                item = "<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td> \
                    <td><a href='{}'>debug</a></td></tr>".format(ami_id, region, name,
                    casename, result, error, debuglog)
                with open(logdir + '/sum.html', 'a+') as fh:
                    fh.write(item)
    with open(logdir + '/sum.html', 'a+') as fh:
        fh.write(html_tail)
